		...
		for (unsigned int i = 0; i < traj.points.size(); i++) {
			...
			for (unsigned int j = 0; j < traj.points[i].positions.size(); j++) {
				...
				new_point.velocities.push_back(
						traj.points[i].velocities[mapping[j]]*0.2);
				if (traj.points[i].accelerations.size() != 0)
					new_point.accelerations.push_back(
							traj.points[i].accelerations[mapping[j]]*0.2*0.2);
			}
			new_point.time_from_start = ros::Duration(traj.points[i].time_from_start.toSec() / 0.2);
